...
from settings import (UI_TEXT_COLOR, GLIM_COST, WELLSPRING_COST, BEACON_COST, 
                      ...
                      MOTION_SKILL_ICON_PATH, DESTROY_ICON_PATH)
...
class UI:
    def __init__(self, game_state, screen_width, screen_height):
        ...
        self.destroy_button_rect = pygame.Rect(0, 0, 60, 60) # New
        ...
        self.hammer_icon = self._load_icon(HAMMER_ICON_PATH, (40, 40))
        self.skill_icon = self._load_icon(SKILL_ICON_PATH, (40, 40))
        self.motion_skill_icon = self._load_icon(MOTION_SKILL_ICON_PATH, (40, 40))
        self.destroy_icon = self._load_icon(DESTROY_ICON_PATH, (40, 40))
    ...
    def handle_click(self, mouse_pos):
        ...
        if self.skill_tree_open:
            if self.motion_skill_rect.collidepoint(mouse_pos): return "purchase_glimversal_motion"
            if self.drills_skill_rect.collidepoint(mouse_pos): return "purchase_glimdraulic_drills"
            if not self.skill_panel_rect.collidepoint(mouse_pos): self.skill_tree_open = False
            return "ui_click"
        
        if self.destroy_button_rect.collidepoint(mouse_pos):
            self.game_state.destroy_mode = not self.game_state.destroy_mode
            return "toggle_destroy_mode"
    ...
    def draw(self, screen, camera_offset_x):
        ...
        self.destroy_button_rect.bottomleft = (self.build_menu_button_rect.right + 10, self.screen_height - 15)
        self._draw_destroy_button(screen)

        if self.game_state.skill_tree_unlocked:
            self.skill_tree_button_rect.bottomright = (self.build_menu_button_rect.left - 10, self.screen_height - 15)
    ...
    def _draw_build_panel(self, screen, mouse_pos):
        ...
        if self.beacon_button_rect.collidepoint(mouse_pos):
            self._draw_info_tooltip(screen, mouse_pos, ["Boosts speed of nearby Glims.", "Effect stacks with diminishing returns."])
    ...
    def _draw_skill_button(self, screen, mouse_pos, skill_name, rect, icon, tooltip_lines):
        ...
        if rect.collidepoint(mouse_pos):
            ...
            if not is_unlocked:
                ...
                lines.extend([f"Cost: {skill['cost_essence']} LE, {skill['cost_sp']} SP", req_glims_str])
    ...
    def _draw_skill_tree_button(self, screen):
        self._draw_icon_button(screen, self.skill_tree_button_rect, "2", None)

    def _draw_build_menu_button(self, screen):
        self._draw_icon_button(screen, self.build_menu_button_rect, "1", self.hammer_icon)

    def _draw_destroy_button(self, screen):
        is_active = self.game_state.destroy_mode
        self._draw_icon_button(screen, self.destroy_button_rect, "Del", self.destroy_icon, is_active)

    def _draw_icon_button(self, screen, rect, key_text, icon, is_active=False):
        mouse_pos = pygame.mouse.get_pos()
        hover = rect.collidepoint(mouse_pos)
        color = UI_BUTTON_HOVER_COLOR if hover or is_active else UI_BUTTON_COLOR
        pygame.draw.rect(screen, color, rect, border_radius=8)
        
        if icon:
            screen.blit(icon, icon.get_rect(center=rect.center))
        
        text_surf = self.font_tiny.render(key_text, True, UI_TEXT_COLOR)
        screen.blit(text_surf, (rect.left + 5, rect.bottom - 15))
